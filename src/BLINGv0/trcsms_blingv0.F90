MODULE trcsms_blingv0
   ...
   INTEGER :: numsms, numsms2, numphp, numine ! logical unit for phosphate budget
...
CONTAINS
   ...
   SUBROUTINE trc_sms_bling( kt )
      ...
      IF( ln_bling_mass .AND. lwp) THEN      !   Write values for phosphate budget
...
wrk1(ji,jj,jk)=gamma_dop*f_dop
      ...
      !trn(:,:,:,jpPO4_bling) = trn(:,:,:,jpPO4_bling) + j_po4(:,:,:)*rfact
      !trn(:,:,:,jpDOP_bling) = trn(:,:,:,jpDOP_bling) + j_dop(:,:,:)*rfact
      !trn(:,:,:,jpFed_bling) = trn(:,:,:,jpFed_bling) + j_fed(:,:,:)*rfact
      !trn(:,:,:,jpOxy_bling) = trn(:,:,:,jpOxy_bling) + j_oxy(:,:,:)*rfact
      !trn(:,:,:,jpine_bling) = 10.e0/(86400.e0*365.e0)*rfact
...
wrk2(:,:,:)=cvol(:,:,:)
   ...
   SUBROUTINE trc_sms_bling_mass_conserv (kt)
      ...
      IF( kt == nittrc000 ) THEN 
         ...
         WRITE(numphp,9500) kt,  glob_sum(cvol(:,:,:))
         WRITE(numine,9500) kt,  glob_sum(cvol(:,:,:))
